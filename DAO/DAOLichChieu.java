...
public class DAOLichChieu implements DAOInterFace<LichChieu> {
    ...
    String sql = "INSERT INTO LichChieu (movie_id, room_id, date, time) VALUES ( ?, ?, ?, ?)";

    try (Connection conn = duLieu.ket_noi()){
        PreparedStatement ptm = conn.prepareStatement(sql) ; 
        ptm.setInt(1, t.getIdPhim());
        ptm.setInt(2, t.getIdPhong());
        ptm.setDate(3, java.sql.Date.valueOf(t.getTg()));  // Nếu tg là LocalDate
        ptm.setTime(4, java.sql.Time.valueOf(t.getTime())); // Nếu time là LocalTime
